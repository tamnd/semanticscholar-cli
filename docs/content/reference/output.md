...
describes what users get. Pick a format with `-o`, or let s2 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
s2 <command> -o table   # aligned columns for reading
s2 <command> -o jsonl   # one JSON object per line, for piping
s2 <command> -o json    # a single JSON array
s2 <command> -o csv     # spreadsheet friendly
s2 <command> -o tsv     # tab-separated
s2 <command> -o url     # just the URL column
s2 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
s2 <command> --fields id,title,url
```
...
```bash
s2 <command>            # a table, because this is a terminal
s2 <command> | wc -l    # JSONL, because this is a pipe
```
